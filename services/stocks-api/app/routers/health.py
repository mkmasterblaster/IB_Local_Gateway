...
async def health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        if ib_client is None:
            ...
            ib_connected = bool(
            ib_client.is_connected()
            if hasattr(ib_client, "is_connected")
            else getattr(ib_client, "connected", False)
        )
...
async def readiness_probe() -> Dict[str, Any]:
    ...
    Ready when DB + Redis are healthy.
    IB status is reported but does NOT block readiness.
    """
    db_healthy = await check_db_health()
    redis_healthy = await redis_client.ping()
    ...
    try:
        ...
        if ib_client is not None:
            ...
            ib_state = "connected" if ib_ok else "disconnected"
    except Exception:
        ib_state = "error"
    ...
    return {
        ...
        "services": {
            "database": {"status": "healthy" if db_healthy else "unhealthy", "level": "ok" if db_healthy else "fail"},
            "redis": {"status": "healthy" if redis_healthy else "unhealthy", "level": "ok" if redis_healthy else "fail"},
            "ib_gateway": {"status": ib_state, "level": "ok" if ib_ok else "warn"},
        }
    }
