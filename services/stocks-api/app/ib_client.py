...
class IBKRClient:
    ...
    async def connect(self) -> bool:
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                # Async-safe handshake
                try:
                    await self.ib.reqCurrentTimeAsync()
                    logger.info("ib_handshake_ok", attempt=attempt)
                except Exception as e:
                    logger.warning("ib_handshake_failed", attempt=attempt, error=str(e))
                ...
                if self.ib.isConnected():
                    # Connection verified - wire handlers and mark as connected
                    self.connected = True
                    from datetime import datetime
                    self.last_connected_at = datetime.utcnow().isoformat() + "Z"
                    self.last_connect_error = None
                    self.last_connect_error_at = None
                    ib_connection_status.labels(container=self.container_name).set(1)
                    logger.info("ibkr_connected", host=self.host, port=self.port, client_id=self.client_id, attempt=attempt)
                    return True
                ...
                self.last_connect_error_at = datetime.utcnow().isoformat() + "Z"
    ...
    async def disconnect(self) -> None:
        ...
        if self.ib and self.ib.isConnected():
            try:
                ...
                ib_connection_status.labels(container=self.container_name).set(0)
                from datetime import datetime
                self.last_disconnected_at = datetime.utcnow().isoformat() + "Z"
    ...
    def _on_exec_details(self, *args):
        """
        Fallback fill handler for ib_insync versions that use execDetailsEvent instead of fillEvent.
        execDetailsEvent signatures vary by version; treat last arg as fill-like.
        """
        try:
            fill_like = args[-1] if args else None
            # Try simplest: existing _on_fill(fill)
            try:
                return self._on_fill(fill_like)
            except TypeError:
                # Some versions pass (trade, fill) or similar
                if len(args) >= 2:
                    return self._on_fill(args[-2], args[-1])
        except Exception as e:
            logger.warning("ib_exec_details_handler_error", error=str(e))
